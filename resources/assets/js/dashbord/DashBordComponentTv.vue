    ...
    <v-layout row wrap >
      <v-flex v-for="dashboarduser in dashboardusers" :key="dashboarduser.id" lg3 md6 xs12>
        <v-card v-bind:class="{ 'green-box': dashboarduser. status === 0, 'yerrow-box': dashboarduser.status === 1 , 'red-box': dashboarduser. status === 2}">
              ...
              <v-tooltip top>
                <template v-slot:activator="{ on }">
                  <v-btn icon  @click="updateUser(dashboarduser)" style="margin:0;padding:0;" v-on="on">
                    <v-icon style="font-size:10px; color:#fff">refresh</v-icon>
                  </v-btn>
                </template>
              ...
              <v-tooltip top>
                <template v-slot:activator="{ on }">
                  <v-btn icon  @click="openLocationModal(dashboarduser);" style="margin:0;padding:0;" v-on="on">
                    <v-icon style="font-size:10px; color:#fff">event</v-icon>
                  </v-btn>
                </template>
              ...
              <v-tooltip top>
                  <template v-slot:activator="{ on }">
                    <v-btn icon @click="openCommentModal(dashboarduser);" style="margin:0;padding:0;" v-on="on" >
                      <v-icon style="font-size:10px; color:#fff">edit</v-icon>
                    </v-btn>
                  </template>
          ...
          <v-divider></v-divider>
          <p style="bacground-color:red; font-size:24px; padding:1px; margin:0; cursor: pointer; color:#fff">{{dashboarduser.location}}</p>
        </v-card>
      </v-flex>
    </v-layout> 
